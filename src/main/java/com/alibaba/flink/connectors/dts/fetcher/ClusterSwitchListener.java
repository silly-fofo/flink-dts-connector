...
/**
 * We recommend user register this listener. Cause when origin cluster is unavailable and new
 * cluster is created by HA(high available service). The cluster name is different. We want warn
 * user that a new cluster is working. The more important thing is that we want user recreate
 * KakfaConsumer and use timestamp to reseek offset. If user following this guid, less duplicated
 * data will be pushed. Otherwise
 */
        ...
        synchronized (this) {
            if (null == originClusterResource) {
                ...
                if (clusterResource.clusterId().equals(originClusterResource.clusterId())) {
                    ...
                    throw new ClusterSwitchException(
                            "Cluster changed from "

...
public class DateTime implements Value<String> {
    ...
    private static Set<String> timeZoneNames = new HashSet<>();
    ...
    protected boolean isEffectiveTimeZone(String timeZone) {
        ...
        if (StringUtils.startsWith(timeZone, "GMT") || StringUtils.startsWith(timeZone, "UTC")) {
            return true;
        }
        char c = timeZone.charAt(0);
        if ('-' == c || '+' == c) {
            for (int i = 1; i < timeZone.length(); ++i) {
                if (('0' > timeZone.charAt(i) || '9' < timeZone.charAt(i))
                        && ':' != timeZone.charAt(i)) {
                    return false;
                }
            }
